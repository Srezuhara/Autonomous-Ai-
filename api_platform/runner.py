"""
api_platform/runner.py  —  v2.4.0
==================================
Changes in this version:
  1. _safe_test_score: returns "0/0 (collection errors)" instead of None
     when tests were attempted but all skipped, so frontend can distinguish
     "not run" from "ran but couldn't generate tests".
  2. All other _safe_* helpers unchanged (already hardened in v2.3).
"""
...
def _safe_test_score(test_results: list, build_id: str) -> str | None:
    """
    Extract test score string from TestResult list.

    Returns:
        "N/M"                      — normal case
        "0/0 (collection errors)"  — tests were attempted but all skipped/failed collection
    ...
    total_passed  = 0
    total_tests   = 0
    any_attempted = False  # True if at least one file was NOT skipped
    ...
    for i, r in enumerate(test_results):
        ...
        logger.info(f"[{build_id[:8]}]   test[{i}] passed={p!r} total={t!r} skipped={skipped}")
    ...
    if total_tests == 0:
        if any_attempted:
            # Tests were attempted but collection errors prevented any from running
            logger.warning(f"[{build_id[:8]}] Tests attempted but 0 collected — likely collection errors")
...
class JobRunner:
    ...
    def _run_build(self, build_id: str, prompt: str):
        ...
        # Extract scores — separate try/except so a scoring bug never fails the build
        review_score = None
        debug_score  = None
        test_score   = None
        intent       = {}
