...
"""
    ...
    def _missing_files(
        ...
        The disk is the authority, not `all_files`. Several files are written
        outside the generation loop and so never enter that list — README.md
        and SETUP.md by this agent, requirements.txt by requirements_builder —
        and every one of them was being reported as never generated, with an
        "Implement `README.md`" todo raised against a finished 46-line file.
        """
        try:
            import config
        ...
        for spec in architecture.get("files", []) or []:
            ...
            if full in written:
                continue
            if base is not None and (base / root / path).is_file():
                continue
            missing.append((path, spec.get("description", "no description")))
        return missing

    _PLACEHOLDER_ITEM = "still contain the scaffold placeholder"
    _PLACEHOLDER_MARKER = "will be generated by the code generation agents"

    def _drop_stale_issues(self, items: list, root: str) -> list:
        """Drop remediation issues that the disk no longer supports.

        The placeholder audit runs at verification time, which is BEFORE the
        documenter writes README.md. Row 3 was degraded to done_with_context
        over a README that this agent had already filled in by the time the
        message was composed — the audit's verdict outlived the condition.
        """
        try:
            import config
            project = Path(config.OUTPUT_DIR) / root
        except Exception:
            return list(items)
        ...
        for item in items:
            text = str(item)
            if self._PLACEHOLDER_ITEM not in text:
                kept.append(item)
                continue
            named = [n.strip() for n in text.rsplit(":", 1)[-1].split(",") if n.strip()]
            still = []
            for rel in named:
                target = project / rel
                try:
                    if target.is_file() and self._PLACEHOLDER_MARKER in target.read_text(
                        encoding="utf-8", errors="ignore"
                    ):
                        still.append(rel)
                except Exception:
                    still.append(rel)          # unreadable: do not silently clear it
            if not still:
                logger.info(
                    f"  ✅ Placeholder issue cleared before reporting: {', '.join(named)}"
                )
                continue
            kept.append(
                f"{len(still)} planned file(s) were never filled in and still "
                f"contain the scaffold placeholder: {', '.join(still)}"
                if len(still) != len(named) else item
            )
        return kept
