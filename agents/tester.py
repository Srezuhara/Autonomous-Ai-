"""
agents/tester.py — Phase 20 (Issue 8 fix)
==========================================
FIX — Issue 8: No import pre-check before test generation.

Root cause in previous versions:
  _test_file() immediately called _generate_tests() regardless of whether
  the source file was actually importable.  A file that survived the debugger
  loop (e.g. because its error was in IGNORE_ERRORS) but still fails a real
  import-check would cause pytest collection to crash with ImportError on
  every generated test, inflating the "0/N tests passing" counter and wasting
  one LLM call per file on dead-end test generation.

Fix applied at the top of _test_file():
  1. Run run_python(file_path) before generating tests.
  2. If it fails AND the error is not in the known-ignorable set, mark the
     TestResult as skipped with skip_reason="import-check failed: <error>".
  3. This means test scores are now authentic: only files that are genuinely
     importable get tested.

Known-ignorable import errors (do not cause skipping):
  - "uvicorn" / "Address already in use" — FastAPI server startup, not an import error
  - "OperationalError" + "sqlalchemy" — DB not running, file still importable

All Phase 16.3 (Vitest) and Phase 15 features retained unchanged.
"""
import logging
import os
import sys
...
GROQ_MAX_TEST_FILES = int(os.getenv("GROQ_MAX_TEST_FILES", "5"))
GROQ_TEST_CODE_CHARS = int(os.getenv("GROQ_TEST_CODE_CHARS", "1400"))
...
class Tester(BaseAgent):
    ...
    def _discover_testable_files(self, file_paths: list[str]) -> list[str]:
        ...
        for fp in candidates:
            if fp not in seen:
                seen.add(fp)
                result.append(fp)
        result = sorted(result, key=_test_priority)
        if GROQ_FREE_TIER_CONSERVE:
            result = result[:GROQ_MAX_TEST_FILES]
        return result
    ...
    def _write_vitest_config(self, root: str) -> None:
        ...
        try:
            ...
            pkg_json_path.write_text(json.dumps(pkg, indent=2, ensure_ascii=False), encoding="utf-8")
        except Exception as e:
...
Pattern (React component with default export):
  ...
  describe('{stem}', () => {{
    it('renders without crashing', () => {{ render(<{stem} />) }})
    it('displays something in the DOM', () => {{ render(<{stem} />); expect(document.body).toBeDefined() }})
  }})
"""
        elif named_exports:
...
Pattern (named export):
  ...
  describe('{stem}', () => {{
    it('exports {first_export}', () => {{ expect({first_export}).toBeDefined() }})
    it('{first_export} returns a value', () => {{ const result = {first_export}(); expect(result).toBeDefined() }})
  }})
"""
        else:
            pattern_hint = f"""
Pattern (module import):
  ...
  describe('{stem}', () => {{
    it('module loads without error', () => {{ expect({stem}Module).toBeDefined() }})
    it('module has expected shape', () => {{ expect(typeof {stem}Module).toBe('object') }})
  }})
"""
...
- Return ONLY the test code. No markdown."""
    ...
    def _run_vitest(self, root: str) -> tuple[str, int]:
        ...
        try:
            result = subprocess.run(
                ...
                capture_output=True, text=True, timeout=90, cwd=frontend_dir,
            )
            output = (result.stdout or "") + (result.stderr or "")
            return output, result.returncode
        except subprocess.TimeoutExpired:
    ...
    def _test_file(self, file_path: str, root: str) -> TestResult:
        ...
        # ── Issue 8 FIX: import pre-check before spending tokens on test gen ──
        # This catches files that have subtle runtime import issues not caught
        # by the debugger (e.g. IGNORE_ERRORS entries that pass debug but
        # would make pytest collection crash with ImportError).
        # We use the same ignorable-error list as the debugger.
        sanity = run_python(file_path)
        if not sanity.success:
            stderr = sanity.stderr or ""
            # Check if the error is one we can safely ignore at import-check time
            is_ignorable = (
                any(p in stderr for p in _IMPORT_CHECK_IGNORABLE)
                or ("sqlalchemy" in stderr.lower() and "OperationalError" in stderr)
            )
            if not is_ignorable:
                # Genuine import failure — skip test generation, save the LLM call
                short_err = stderr[:120].replace("\n", " ")
                result.skipped     = True
                result.skip_reason = f"import-check failed: {short_err}"
                logger.info(
                    f"  ⏭️  [{file_path}] Skipping test generation — "
                    f"import-check failed: {short_err}"
                )
                return result
        # ── End Issue 8 fix ───────────────────────────────────────────────────
...
PATTERN C — Pure Pydantic models:
  import {stem}
...
FILE: {file_path}
CODE:
{code[:GROQ_TEST_CODE_CHARS]}{routes_context}{main_context}{sibling_ctx}{mock_examples}
...
CRITICAL: Never use dotted module paths. Fix ONLY the imports.
