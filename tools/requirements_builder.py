...
# The architect scaffolds requirements.txt with a comment block ending in this
# marker (agents/architect.py:PLACEHOLDER_MARKER). Appending real packages below
# it leaves the marker in place, and pipeline._audit_placeholders then reports
# the file as "never filled in" -- degrading an otherwise clean build to
# done_with_context over a file that is, by then, complete.
PLACEHOLDER_MARKER = "will be generated by the code generation agents"


def _strip_scaffold_placeholder(text: str) -> str:
    """Drop the architect's placeholder comment block from requirements text.

    Removes the marker line together with the contiguous comment lines directly
    above it (the description half of the block). Anything else -- including
    comments a generator wrote deliberately -- is left untouched.
    """
    lines = text.splitlines()
    marked = [i for i, line in enumerate(lines) if PLACEHOLDER_MARKER in line]
    if not marked:
        return text
...
def _implied_packages(package_names: set) -> list:
    """Packages a project needs to *run* but never imports.

    A generated FastAPI app is started with `uvicorn backend.main:app` -- the
    command every SETUP.md emits -- yet nothing in the source imports uvicorn,
    so the import scan alone ships a requirements.txt that cannot serve the app.
    """
    implied = []
    if "fastapi" in package_names and "uvicorn" not in package_names:
        implied.append(IMPORT_TO_PYPI["uvicorn"])
    return implied
...
def validate_and_fix_requirements(
    ...
    final_names = existing_normalised | {
        _normalise(re.split(r"[><=!\[;]", spec)[0].strip()) for spec in to_add
    }
    for spec in _implied_packages(final_names):
        to_add.append(spec)
    ...
    try:
        # The file is about to hold real packages, so it is no longer a stub.
        existing_text = _strip_scaffold_placeholder(existing_text)
